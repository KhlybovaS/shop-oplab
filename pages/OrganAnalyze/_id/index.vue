  ...
  <div class="main-content">
    <div class="card">
      <h5 class="title is-5 card-header">
        ...
        <div class="card-content" v-for="item of sortedItems.filter(item => item.OrganId ==  [$route.params.id]  )" :key="item.Id">
            <div>
...
<script>
  ...
  export default {
    ...
    asyncData ({ params }, callback) {
      ...
      let post = posts.find(post => post.OrganId === parseInt(params.id))
      if (post) {
        callback(null, { post })
      } else {
    ...
    head () {
      return {
        title: this.post.Organ,
        meta: [
          {
            hid: 'description',
            name: 'description',
            content: this.post.Organ
          }
        ]
      }
    },
